...
Glidein is a pilot job based system described in full in \cite{frey2002condor}.  The factory uses glideins on non-condor resources in order to create a virtual condor cluster that can transfer jobs with flocking.  The glidein job is a few condor daemons packaged and started when the job starts.  At startup, the glidein reports to the factory the specifications of the machine.  The glidein is then available for jobs to execute.

Glideins are being used in production in the Open Science Grid because they provide several advantages over regular job submission.  Each glidein sandboxes and monitoring the user jobs it runs.  The glidein can run multiple user jobs inside a single LRMS job, and will continue to run until the configured time to stop.  And the glidein connects directly with the submitter host, removing the need for staging data.
