...
Another usage metric is installation of the Campus Grid Factory.  Currently, the campus grid factory is used in production only on Firefly at Nebraska.  But, it is currently being run in test environments at the National Center for Supercomputing Applications (NCSA) \cite{ncsa}.  At the NCSA, users of the machines have installed the CGF to flock from their submission host at their home institution, the Renaissance Computing Institute \cite{renci}.  Additionally, campus grids such as Louisiana Tech and the Sunshine Grid in Florida are experimenting with the framework.
