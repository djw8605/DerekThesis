...
At many universities, each department maintains a independent cluster for their researchers.  Many of these clusters are under utilized while researchers at other departments are resource starved.  In this paper we will describe a framework and technologly that are used to link departmental clusters such that submission at one cluster could lead to execution on another.  This framework is then further expanded to operating on a national production grid.  All of these components a functional and are running research jobs in production.
...
\item Researchers have peaks in usage and need overflow capacity, while maintaining a single execution environment.  This is an expressed goal of the Condor project.
