...
\begin{itemize}
\item Removes gateway bottleneck by transferring files directly from submitter to execute host.
...
\item Greatly reduces the work that the gateway needs to perform.  The authentication and authorization work is all done by the execute and submission host.
\item Not reliant on the reliability or bandwidth of the shared filesystem.  Further, the submitter is relatively unaffected other users that are not running on the same submission host.
\end{itemize}
